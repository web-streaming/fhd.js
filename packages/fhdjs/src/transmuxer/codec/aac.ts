...
export class AAC {
  ...
  static parseAdts(data: Uint8Array, pts: number) {
    ...
    while ((i + 2) < len) {
      if (data[i] === 0xff && (data[i + 1] & 0xf6) === 0xf0) break;
      i++;
    }
    ...
    const { config, codec } = AAC._getConfig(samplingFrequencyIndex, channelCount);
  ...
  static parseAudioSpecificConfig(data: Uint8Array) {
  ...
  static _getConfig(samplingIndex: number, channelCount: number) {
    const config = [];
    let objectType;
  ...
  static getSilentFrame(codec?: string, channelCount?: number): Uint8Array | void {
    switch (codec) {
      case 'mp4a.40.2':
        if (channelCount === 1) {
