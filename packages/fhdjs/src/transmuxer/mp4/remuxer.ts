...
export class MP4Remuxer {
  ...
  remux(needInitSegment = false) {
    ...
    videoTrack.duration = videoTrack.duration || 0xffffffff;
    audioTrack.duration = audioTrack.duration || 0xffffffff;
    ...
    if (hasVideo && videoTrack.sampleSize) videoSegment = this.remuxVideo();
    if (hasAudio && audioTrack.sampleSize) audioSegment = this.remuxAudio();
