...
export class M3U8Parser {
  static parse(text: string, parentUrl: string, rendition: HlsRendition, stream?: HlsStream): HlsRendition | void {
    if (!text) return;
    const lines = getLines(text);
    if (!lines.length || !lines.includes('#EXTM3U')) return;

    if (text.indexOf('#EXTINF:') > 0 || text.indexOf('#EXT-X-TARGETDURATION:') > 0) {
      return this.parseMedia(lines, parentUrl, rendition, stream);
    }

    return this.parseMaster(lines, parentUrl, rendition);
  }

  private static parseMaster(lines: string[], parentUrl: string, rendition: HlsRendition): HlsRendition {
  ...
  private static parseMedia(lines: string[], parentUrl: string, rendition: HlsRendition, stream?: HlsStream): HlsRendition {
    if (!stream) {
