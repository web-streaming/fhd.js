import { findChildren, parseDuration } from './helper';
...
export class MPDParser {
  ...
  private static parseAdaptationSet(node: Element) {
    findChildren(node, 'Representation').map((x) => MPDParser.parseRepresentation(x));
  }

  private static parseRepresentation(node: Element) {
